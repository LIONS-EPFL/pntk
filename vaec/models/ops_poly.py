...
def conv2d(input, kernel_sizes, N_channels, stride_sizes,padding='same',
    ...
    with tf.variable_scope(scope, reuse=reuse) as scope:
        shortcut = tf.layers.conv2d(input, 32, 4, 2, padding='same', activation=activation_fn,
        ...
        for l in range(len(N_channels)):
            ...
            with tf.variable_scope(layer_name) as scope:
                output = tf.layers.conv2d(input, int(N_channels[l]), int(kernel_sizes[l]), int(stride_sizes[l]),
                                            ...
                                            kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                            bias_initializer=tf.constant_initializer(bias_val))
                if l != 0: output += output * tf.keras.layers.AveragePooling2D((2 ** l, 2 ** l))(shortcut)
                input = output
    return output
...
def mlp(input, mlp_size,batch_norm=False, scope='mlp', reuse=True, activation_fn=tf.nn.relu):
    with tf.variable_scope(scope, reuse=reuse) as scope:
        ...
        for l in range(len(mlp_size)):
            layer_name = 'layer_' + str(l + 1)
            if l == 0:
                output, w, biases = fc_layer(input, mlp_size[l], batch_norm=batch_norm, scope=layer_name,
                                                reuse=reuse, activation_fn=activation_fn)
            else:
                output, w, biases = fc_layer(output, mlp_size[l], batch_norm=batch_norm, scope=layer_name,
                                                reuse=reuse, activation_fn=activation_fn)
