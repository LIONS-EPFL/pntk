...
def sample_ball(radius, x_dim):
    u = np.random.normal(0, 1, x_dim) #shape:(2,)
    u = u / LA.norm(u)
...
def sample_sphere(type,data_range):
    r2 = np.sqrt(data_range) * np.sqrt(np.random.rand(1))

    if type == "all":
        theta2 = 2 * np.pi * np.random.rand(1)
    if type == "p":
        theta2 = 2 * np.pi * np.random.rand(1)*0.5
    if type == "2p":
        theta2 = 2 * np.pi * np.random.rand(1)*0.25
    x2 = r2 * np.cos(theta2)
    y2 = r2 * np.sin(theta2)
...
def process_data(args, A, AB, b, W, a, data_range, n_data, noise, shape, sign, fix):
    ...
    if args.data == 'sqrt':
        sign = 'p' #!!!of course, need to be positive
        fix = args.x_dim
    ...
    while data_cnt < n_data:
        if shape == 'cube':
            if sign == 'z':
                ...
                x0 = sample_cube(0.0, data_range, fix)
                x1 = sample_cube(0.0, data_range, args.x_dim-fix)
                x = np.concatenate((x0, x1))
            elif sign == 'n':
            ...
            if sign == 'z':
                pass
                # x0 = [0.1]
                # x1 = sample_sphere(-data_range, data_range, args.x_dim-1)
                # x = np.concatenate((x0, x1))
            elif sign == 'p':
                x = sample_sphere("p",data_range)
            elif sign == '2p':
                x = sample_sphere("2p", data_range)
            else:
                x = sample_sphere("all", data_range)
        elif shape == 'basis':
            if turn == 1:
                x = np.zeros(args.x_dim)
                x[idx] = data_range #data_range: 5   x: (32,)
                prev_x = x
                x = np.matmul(args.Q, x) #args.Q: 32*32
            else:
                x = prev_x
                x = -x
        ...
        if args.data=='linear':
        ...
        elif args.data=='cospix':
            y = [np.cos(x * np.pi).sum()]
        elif args.data=='cosx':
            y = [np.cos(x).sum()]
        elif args.data=='cos2pix':
            y = [np.cos(x * np.pi*2).sum()]
        elif args.data=='cos10x':
            y = [np.cos(10*x).sum()]
        elif args.data=='sin':
            ...
            y = [np.sum(np.square(x))] #original
            #y = np.matmul(A, np.square(x)) + b
        elif args.data == 'x2':
            y = np.power(x, 2)
        elif args.data == 'helix':
            y1 = 1 * np.cos(x * 2 * np.pi);
            y2 = 1 * np.sin(x* 2 * np.pi);
            y = [y1,y2]
        elif args.data == 'x3x2minus10x1plus5':
            y = np.power(x, 3) +np.power(x,2)- 10* x +5
        elif args.data == 'x4x2bias1':
            y = np.power(x, 4) + np.power(x, 2) +1
...
def sample_data(args):
    A = sample_A(args)
    AB = sample_AB(args)
    W, a = sample_WA(args) #W: np.random.uniform
    ...
    test = process_data(args, A, AB, b, W, a, args.test_r, args.n_test, args.noise, args.test_shape, 'no', 0) #:'no',0
    # if args.data=='linear' and args.x_dim==2:
    #     #record what is the matrix A
    #     output = '%s/%s_xdim%s_item%s_trains%s_tests%s_sign%s_fix%s_testr%s_trainr%s_valr%s_ntrain%s_nval%s_ntest%s_Ar%s_br%s' % (
    #         args.folder, args.data, args.x_dim, args.n_item, args.train_shape, args.test_shape, args.sign, args.fix,
    #         args.test_r, args.train_r, args.val_r, args.n_train, args.n_val, args.n_test, args.A_r, args.b_r)+ str(A[0][0]) + str(A[0][1]) + ".pickle"
    # else:
    output = '%s/%s_xdim%s_item%s_trains%s_tests%s_sign%s_fix%s_testr%s_trainr%s_valr%s_ntrain%s_nval%s_ntest%s_Ar%s_br%s.pickle' %(
        args.folder, args.data, args.x_dim, args.n_item, args.train_shape, args.test_shape, args.sign, args.fix,
        args.test_r, args.train_r, args.val_r, args.n_train, args.n_val, args.n_test, args.A_r, args.b_r)
