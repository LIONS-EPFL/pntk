...
def main():
    ...
    parser.add_argument('--degree_',type=int)
    parser.add_argument('--log', help='path to the training log')
    parser.add_argument('--device', type=int, default=0, help='which gpu to use if any (default: 0)')
    args = parser.parse_args()
    #args.log = args.path.split("")
    #args.path.replace("data", "models_dir")
    modelpath = join(args.path,"model_best.pth.tar")
    logpath=glob.glob(join(args.path,"*log"))[0]
    ...
    # Load model
    model = pinet(args).to(args.device)
    ...
    for i in range(n_trials):
        ...
        k = find_k(w, r, shape)  #r: 1   shape: "sphere"    k =adjust * r = 1.1

        sign_x = w.sign()
        w_abs = w.abs()

        ks = torch.Tensor(np.linspace(0, extend_ratio*data.max().item(), num=n_sample))\
                 .reshape(n_sample) + k  #n_sample: 100   extend_ratio: 10
        ks = ks.T
        ...
        degree_ = args.degree_
        
        for power in range(2,degree_+1,1):
            X = np.concatenate([X,X**power],-1)
